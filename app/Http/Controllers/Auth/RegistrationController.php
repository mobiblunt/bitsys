    ...
    protected function postRegister(Request $request)
    {
        ...
        */
        ...
        if ($request->get('plan_id') == "1") {
           ...
           $duration = $currentdate->addDays(1)->toDateString();
           
        }
        elseif ($request->get('plan_id') == "2" || $request->get('plan_id') == "5") {
            $duration = $currentdate->addDays(3)->toDateString();
        }
        elseif ($request->get('plan_id') == "3") {
            $duration = $currentdate->addDays(5)->toDateString();
        }
        elseif ($request->get('plan_id') == "4") {
            $duration = $currentdate->addDays(7)->toDateString();
        }
        
        else {
            $duration = $currentdate->addDays(0)->toDateString();
        }
        ...
        Mail::queue(
                ...
                function ($message) use ($email) {
                    $message->to($email)
                        ->subject('Welcome To Forbes Coins!');
                }
            );
